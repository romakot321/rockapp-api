...
class GoogleDetectorAdapter[TDetection: DetectionDTO, TAdditional: None](IDetectClient):
    def __init__(self, queue_service: IQueueService, storage: IStorage) -> None:
        self.queue_service = queue_service
        self.storage = storage

    async def create_detection(self, detection_id: UUID, image: BinaryIO, additional_data: None = None) -> DetectionDTO:
        self.storage.store_file(str(detection_id), image)
        job = self.queue_service.enqueue_job(RunDetectUseCase.execute_google, str(detection_id), additional_data, job_id=str(detection_id))
